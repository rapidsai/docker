...
docker run --gpus all --rm -it \
    ...
    -u $(id -u) \
    rapidsai/cuvs-bench:25.08a-cuda12.8-py3.12 \
...
- `rapidsai/cuvs-bench:25.08a-cuda12.8-py3.12`: Image to use, either `cuvs-bench` or `cuvs-bench-datasets`, cuVS version, CUDA version, and Python version.
...
docker run  all --rm -it \
    -v $DATA_FOLDER:/home/rapids/benchmarks \
    -u $(id -u) \
    rapidsai/cuvs-bench-cpu:25.08a-py3.12 \
     "--dataset deep-image-96-angular" \
     "--normalize" \
     "--algorithms cuvs_cagra" \
     ""
